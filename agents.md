...
- **tapp** ([amds-ldeo/tapp](https://github.com/amds-ldeo/tapp)) -- the authoritative TAPP source library (Ruolin's tables + modules + `composed_tapps.json`), vendored here as the **`tapp/` git submodule**. `tools/tapp_source.py:current_delivery()` resolves to it (falling back to any inline `TAPPS<date>/` drop). Clone with `--recursive` / `git submodule update --init`; bump the delivery by advancing the submodule commit. The tables are read-only to this repo — never edit them here.
