...
`_sources/geochemProperties/detailEMPA/` carries per-dataset values that complement an empaTAPP TAPP definition. Schema is hand-authored (`ada:componentType` enum, `schema:measurementTechnique` `anyOf` pointing at empaTAPP) plus `allOf [- $ref: parametersConstraint.yaml]` to fold in the generated `schema:additionalProperty[]` `oneOf` constraint.

- **Generated artifacts:** `parametersConstraint.yaml`, plus paired `exampledetailEMPA-P{N}.json` instances (one per pub).
- **Hand-authored bits:** `schema.yaml`'s `ada:componentType` enum and `schema:measurementTechnique` `anyOf` are user-maintained. The build_detail_BB.py scaffolder writes a stub on first run (when no `schema.yaml` exists yet) with placeholder `ada:TODO_ComponentType` consts.

## Phase D (analytes → variableMeasured) — deferred

The runtime transformation from a TAPP instance's `ada:analyteTemplate.ada:defaultAnalytes[]` rows to `schema:variableMeasured[]` entries (typed as `schema:PropertyValue + cdi:InstanceVariable`) on an adaEMPA / empaProfile dataset record is **deferred** to the ada_metadata_forms repo (Django app). Schema enforcement on this repo's side is structural only; the population is generator-driven at record-creation time. See the "Pending Work — TAPP/detailEMPA Integration" section in `~/OneDrive/Documents/GithubC/amds-ldeo/metadata/ada_metadata_forms/agents.md`.
