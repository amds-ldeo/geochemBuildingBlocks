...
description: Electron-probe microanalysis (EPMA/EMPA, WDS/EDS) extension of the base
  TAPP definition, generated from TAPPS20260813/Current TAPPs/EPMA_TAPP_v20.csv via
...
- $ref: https://raw.githubusercontent.com/amds-ldeo/geochemBuildingBlocks/undefined/build/annotated/BaseSchema/modules/reportingCore/schema.yaml#/$defs/AllBlocks_Procedure
- type: object
  properties:
    schema:instrument:
      type: array
          ...
          then:
            properties:
              schema:model:
                ...
                properties:
                  schema:name:
                    description: Model of the EPMA instrument.
                    type: string
                    enum:
                    - JXA-8530F Plus
                    - JXA-8230
      ...
      items:
        ...
        - if:
          ...
          then:
            properties:
              schema:name:
                description: Data acquisition and automation software, including version
                  number.
                anyOf:
                - type: string
                - type: array
        ...
        - if:
          ...
          then:
            properties:
              schema:name:
                description: Software used for matrix correction and quantitative
                  data reduction, if separate from the acquisition software. For mapping
                  workflows, include software used to process count maps into concentration
                  maps.
                anyOf:
                - type: string
                - type: array
        ...
        ada:analyteColumns:
          ...
          items:
            ...
            - title: Detection Limit
              description: Method detection limit at 99% confidence (3-sigma), one
                per reported concentration variable (one per analyte, these being
                the same set), derived from counting statistics on peak and background.
                Include the method and the resulting value for each.
              type: object
              properties:
                '@id':
            ...
            - title: Detection Limit Method
              description: Formula or approach used to calculate detection limits.
                Reference method publication if applicable. When the procedure does
                not specify a method, the analyst should complete this field.
              type: object
              properties:
                '@id':
            ...
            - title: Normalization / Standards-Based Correction
              description: Post-acquisition normalization applied using secondary
                reference materials to correct for session-to-session calibration
                drift.
              type: object
              properties:
                '@id':
    ...
    schema:additionalProperty:
      ...
      items:
        ...
        - title: Analyte Estimation Method
          description: Whether elemental concentrations were calculated directly from
            measured X-ray intensities, or estimated by cation stoichiometry (e.g.,
            oxygen calculated from cation proportions in silicates; carbon from stoichiometry
            in carbonates).
          type: object
          properties:
            '@id':
        ...
        - title: Interfering Elements
          description: Element(s) whose X-ray lines overlap with the measured peak
            for this analyte, requiring a correction.
          type: object
          properties:
            '@id':
    ...
    ada:analyticalMode:
      ...
      items:
        description: Primary analytical mode(s) executed under this procedure. For
          single-mode procedures, records one value (e.g., WDS Point Analysis). For
          multi-mode procedures (e.g., simultaneous WDS point analysis and EDS mapping),
          list all applicable modes. Serves as the procedure-level declaration of
          measurement type, distinct from the mode flag columns which indicate per-field
          applicability.
        type: string
        enum:
        - WDS Point Analysis
    ...
    ada:edsLiveTimePerPointOrPixelDefault:
      ...
      - type: number
      - type: string
    ada:interferenceCorrectionsApplied:
      description: Whether a spectral interference correction was applied for each
        analyte. Common interferences include Ti Kb on V Ka, Cr Kb on Mn Ka, and Ba
        La on Ti Ka.
      type: boolean
      readOnly: true
    ada:samplingUnit:
...
* YAML version: [schema.yaml](https://raw.githubusercontent.com/amds-ldeo/geochemBuildingBlocks/undefined/build/annotated/techniqueProfile/geochemProfile/EMPA/tapp/schema.json)
* JSON version: [schema.json](https://raw.githubusercontent.com/amds-ldeo/geochemBuildingBlocks/undefined/build/annotated/techniqueProfile/geochemProfile/EMPA/tapp/schema.yaml)
...
[context.jsonld](https://raw.githubusercontent.com/amds-ldeo/geochemBuildingBlocks/undefined/build/annotated/techniqueProfile/geochemProfile/EMPA/tapp/context.jsonld)
