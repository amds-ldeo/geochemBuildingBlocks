...
def main():
    ...
    # DEPRECATED GENERATOR — its schema template emits the OLD object-form ada:componentType
    # ({"@type": "..."} nested under schema:distribution), which was migrated to a string long ago.
    # The current profile-ada schemas (correct string form) are produced by tools/_tapp_lib.py.
    # Running this would overwrite them with the deprecated shape, so generation is blocked. The
    # config/listing (`--list`) is kept for reference. Pass --force-deprecated only if you have
    # deliberately re-verified the template against the current string-based componentType model.
    if "--force-deprecated" not in sys.argv:
        print("ERROR: generate_profiles.py is DEPRECATED (emits old object-form ada:componentType).\n"
              "       profile-ada schemas are now generated by tools/_tapp_lib.py (string form).\n"
              "       Use --list to inspect config, or --force-deprecated to override intentionally.",
              file=sys.stderr)
        sys.exit(2)
