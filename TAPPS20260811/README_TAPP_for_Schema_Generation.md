...
| **Analysis** | One execution of that procedure — this sample, this session, this date. | Every time the procedure is run |
...
|---|---|---|
| `EPMA/EPMA_TAPP_v13.csv` | Group1, ReportingCore | `all` |
| `SEM/SEM_TAPP_v10.csv` | Group1, ReportingCore | `all` |
| `SEM/SEM_Composition_TAPP_v10.csv` | Group1, ReportingCore | `all` |
| `SEM/SEM_Imaging_TAPP_v9.csv` | Group1, ReportingCore | `target_selection` |
| `SEM/SEM_FIBSEM_TAPP_v9.csv` | Group1, ReportingCore | `target_selection` |
| `TEM/TEM_TAPP_v13.csv` | Group1, ReportingCore | `target_selection,calibration_factor,aggregation,aggregation_qc` |
| `XCT/Lab-XCT_TAPP_v13.csv` | Group1, ReportingCore | `target_selection,calibration_factor` |
| `LA-Q-ICP-MS/LA-Q-ICP-MS_TAPP_v11.csv` | Group1, LaserAblation, ReportingCore | `all` |
| `LA-SF-ICP-MS/LA-SF-ICP-MS_TAPP_v11.csv` | Group1, LaserAblation, ReportingCore | `all` |
| `LA-MC-ICP-MS/LA-MC-ICPMS_TAPP_v7.csv` | Group1, LaserAblation, MCICPMS, ReportingCore | `all` |
| `Solution Q-ICP-MS/Solution_Q-ICP-MS_TAPP_v13.csv` | Group1, ReportingCore, SolutionIntroduction | `calibration_factor,blank,aggregation,aggregation_qc` |
| `Solution SF-ICP-MS/Solution_SF-ICP-MS_TAPP_v13.csv` | Group1, ReportingCore, SolutionIntroduction | `calibration_factor,blank,aggregation,aggregation_qc` |
| `Solution MC-ICP-MS/Solution_MC-ICP-MS_TAPP_v11.csv` | Group1, MCICPMS, ReportingCore, SolutionIntroduction | `calibration_factor,blank,aggregation,aggregation_qc` |
| `LA-Q-ICP-MS/LA-Q-ICP-MS_UPb_TAPP_v12.csv` | Group1, LaserAblation, ReportingCore, Geochronology, UPb | `all` |
| `LA-SF-ICP-MS/LA-SF-ICP-MS_UPb_TAPP_v12.csv` | Group1, LaserAblation, ReportingCore, Geochronology, UPb | `all` |
| `LA-MC-ICP-MS/LA-MC-ICPMS_UPb_TAPP_v7.csv` | Group1, LaserAblation, MCICPMS, ReportingCore, Geochronology, UPb | `all` |
...
|---|---|---|
| `(none)` | Scalar — one value per procedure/analysis. ~79% of fields. | Ordinary property on the parent object |
| `analyte` | One value per chemical species determined | Property of an object in the `analytes` array |
...
| `A > B` | Containment — B exists only within A | Nested array. Currently unused in the library |
| `pair: A` | Keyed by an unordered pair of A | Property on a pair object, e.g. `{"between": ["206Pb/238U date", "207Pb/235U date"], "value": …}` |

Six keys are in use library-wide: `sampling unit`, `reported property`, `channel`, `analyte`, `standard`,
`preparation step`. Four more (`conversion`, `acquisition pass`, `background position`,
`model component`) are documented but retired from use — you may ignore them.
...
(none)
analyte                            channel
reported property                  sampling unit
preparation step
defines: analyte                   defines: channel
defines: reported property         defines: sampling unit
defines: standard                  defines: preparation step
pair: reported property
sampling unit x analyte            sampling unit x reported property
standard x reported property
```

Sixteen distinct strings across the whole library — small enough to handle as a closed set.
...
From `LA-SF-ICP-MS_UPb_TAPP_v10.csv`:

| Field | Keyed By |
|---|---|
| `Procedure Name` | `(none)` |
| `Analyte` | `defines: analyte` |
| `Monitored Isotopes` | `defines: channel` |
...
```jsonc
{
  ...
  "channels":  [                                         // domain from `Monitored Isotopes`
    { "id": "206Pb", "dwellTimePerMass": "10 ms" },       // `Dwell Time per Mass` → channel
    { "id": "238U",  "dwellTimePerMass": "6 ms" }
  ],
...
## 10. Four universal fields worth special handling
...
| File | Contents |
|---|---|
| `Claude Skills for TAPP/references/conventions.md` | The specification. Rules 1–10; Rule 7 covers `Keyed By` |
