...
**Always read `TAPP_Planning_Table.csv` (in the project root) before beginning Phase 0 for any technique.** The Notes column records deferred decisions, open questions, and scope constraints from prior planning sessions — including decisions about adjacent techniques, mode-bundling choices, and items explicitly flagged for review at TAPP development time. Do not skip this step: some rows have DEFERRED flags that directly constrain Phase 0 decisions.
