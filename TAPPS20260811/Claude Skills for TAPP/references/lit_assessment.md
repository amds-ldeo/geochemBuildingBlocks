...
**Trigger:** Read this file at the start of any Phase 3 literature assessment task — whenever you are asked to extract procedure metadata from one or more papers into a TAPP assessment column.
...
| Value | Use when |
|-------|----------|
| `N` | The field is applicable to this procedure but the value is not directly stated in the paper |
| `N/A` | The concept genuinely does not apply to this procedure (e.g., EDS acquisition time for a WDS-only instrument; halogen correction for a procedure measuring no halogens) |
...
**Column 1 (Metadata Item):** Exactly match the field names in the TAPP (Column A). Include group header rows (`1. Procedure Identification`, etc.) and blank separator rows.

**Group header rows:** Use `N` in all procedure columns (consistent with TAPP convention).
...
## Procedure Separation Decisions
...
appear in each paper and how well documented each technique's procedure is.
...
| `Brief` | Technique is mentioned (possibly with instrument name or facility) but no procedure parameters are given; not useful as a TAPP source on its own |
...
- Common knowledge about what procedures usually contain
...
| Pitfall | Prevention |
|---------|-----------|
| Recording PAP for Caltech GPS Chi Ma procedures | Chi Ma's Caltech lab uses CITZAF (Armstrong 1995), not PAP. This is confirmed in Ma et al. 2017/2018. Check every Ma/Caltech extraction. |
